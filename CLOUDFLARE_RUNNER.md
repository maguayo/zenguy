...
**Modo "el test manda" (27-08, decisión de Marcos):** el gate de acciones irreversibles del ledger (commit `1e34b23`) se desactiva en producción con la var del Worker `RUNNER_UNRESTRICTED_ACTIONS=true` → `ZENGUY_UNRESTRICTED_ACTIONS` en el contenedor. Con el flag activo el runner ejecuta lo que digan las instrucciones del test **sin** allowlist de dominios, ledger de acciones ni gate de HTTP mutante: clicks de botón/submit, navegación a la pasarela, POST de checkout, input de tarjeta. Se mantienen las barreras de INTEGRIDAD (no de política): anti-SSRF/rebinding, redacción de secretos, topes de 32 MiB/256 MiB, descargas denegadas, sandbox de Chromium y el set sin `evaluate`/`send_keys`/subida de ficheros. Reversible con un solo `wrangler secret`/deploy de la var, sin reconstruir la imagen. Nota operativa: con el flag ON, el run horario ejecuta un checkout real cada hora — usar tarjeta de test de Stripe (`4242…`) o modo test en los Secrets del test.
