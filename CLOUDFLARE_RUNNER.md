...
**Incidente (01/02-09, ~9 h sin tests):** el commit `f2a24d0` (preparación de App Review) cambió `RUNNER_DISPATCH` a `queue` en producción y staging asumiendo un «runner privado por cola» con modelo local que no existe desde el 25-08 (`zenguy-runs`: 1 productor, 0 consumidores). Cada attempt se publicó en el vacío, el sweep horario lo marcó `WORKER_LOST` a los 30 min, los 2 retries de infraestructura lo devolvieron a la misma cola y cada run acabó `SYSTEM_ERROR` a las ~3 h (3 runs perdidos entre las 22:20 y las 07:40 CEST). Arreglo (02-09): `container` restaurado en config, bootstrap y tests; el guard post-deploy `verify-app-store-remotes.mjs` exige ahora `runnerDispatch: "container"`; y el claim `cf` de un workspace sin consentimiento de IA remota cierra el attempt al instante como `SYSTEM_ERROR/REMOTE_AI_CONSENT_REQUIRED` en vez de dejarlo QUEUED. Ojo: desde ese commit el consentimiento por workspace (`workspace_remote_ai_consents`, `PUT /api/workspaces/:id/remote-ai-consent`) es obligatorio para que un browser test se ejecute, y se concede desde Workspace Settings → «AI data sharing» en la web (commit posterior al de este arreglo) o desde More → AI data sharing en la app iOS.

**VERIFICADO END-TO-END (27-08):** run manual PASSED en cocunat.com — home (≥4 productos), ficha (173,97 €/289,95 €/−40 %), añadir al carrito, y **checkout con totales verificados en secure.cocunat.com** (subtotal/total 173,97 €, ahorros 130,64 €). 7 pasos. Piezas que lo hicieron posible, en orden de descubrimiento: modelo de red first-party+funcional con trackers cortados; allowlist funcional (workers.dev/pages.dev, shopifycdn.com, config-security bot-protection); contenedor **standard-4 (4 vCPU)** + `TIMEOUT_BrowserStateRequestEvent=90` (la home pesada —56 MB media— reventaba el build del DOM en 2 vCPU); `TIMEOUT_NavigateToUrlEvent=60`; y clave final: en modo unrestricted el `SecurityWatchdog` de browser-use se desactiva (`allowed_domains=None`) porque bloqueaba la navegación al subdominio de checkout — el guard CDP es la única frontera de red. Diagnóstico vía `guard_blocked_hosts_summary` (un log resumen por attempt).

**Modo "el test manda" (27-08, decisión de Marcos):** el gate de acciones irreversibles del ledger (commit `1e34b23`) se desactiva en producción con la var del Worker `RUNNER_UNRESTRICTED_ACTIONS=true` → `ZENGUY_UNRESTRICTED_ACTIONS` en el contenedor. Con el flag activo el runner ejecuta lo que digan las instrucciones del test (clicks de botón/submit, navegación a la pasarela, POST de checkout, input de tarjeta) en **cualquier sitio y cualquier pasarela de pago, sin declarar dominios**.

Scoping de red (aprendido a base de romperlo): con la red 100% abierta el JS de los trackers de terceros corre en el hilo de la página y ahoga el build del árbol DOM de browser-use → captura de estado 30 s/paso, DOM vacío, el attempt se corta a mitad del recorrido (medido; no lo arreglan pool DNS, subir timeouts ni quitar screenshots — solo cortar el JS de terceros: 0,33 s/paso vs 30 s). Modelo final en `browser_worker.py`: navegaciones (Document/iframe, aislados por `--site-per-process`) a cualquier host; **subrecursos** permitidos si son first-party del sitio o del recorrido (`journey_domains`, auto-expandido al navegar) o infra funcional (`FUNCTIONAL_THIRD_PARTY_SUFFIXES`: e-commerce/pago/captcha/CDN/fuentes); el resto —ad/tracker de terceros— se corta con `failRequest`. Añadido: pool de hilos dedicado para `getaddrinfo` (evita el starvation que colgaba el contenedor). Barreras de INTEGRIDAD intactas: anti-SSRF/rebinding, redacción de secretos, topes 32 MiB/256 MiB, descargas denegadas, sandbox, set sin `evaluate`/`send_keys`/upload. Reversible flipando la var. Nota operativa: con el flag ON el run horario ejecuta un checkout real cada hora — usar tarjeta de test de Stripe (`4242…`) o modo test en los Secrets del test.
