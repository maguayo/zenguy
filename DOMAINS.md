...
- **Workers Routes (11 en la zona)**: `*/*` → `zenguy-api-production`, la que captura los dominios de clientes — **declarada en `wrangler.jsonc` (env production)** porque cada deploy reemplaza el conjunto de routes del script y, si no estuviera en la config, el siguiente `pnpm deploy production` la borraría en silencio. Exclusiones con Worker=None creadas en el dash (wrangler no puede crearlas): `zenguy.com/*`, `www.zenguy.com/*`, `app.zenguy.com/*`, `admin.zenguy.com/*`, `staging-app.zenguy.com/*`. Routes declaradas en config: `app…/api/*` y `app…/status/*` → production; `staging-app…/api/*`, `staging-app…/status/*` y **`api-staging.zenguy.com/*`** → staging. **Las Workers Routes se evalúan antes que los Custom Domains** (doc de Cloudflare: "Routes … take precedence if configured on the same hostname"), así que un Custom Domain por sí solo no protege a su worker del wildcard: `admin.zenguy.com` necesita su exclusión y `api-staging.zenguy.com` su route explícita hacia `zenguy-api-staging`. `api.zenguy.com` no necesita nada porque wildcard y Custom Domain terminan en el mismo worker.
...
- **Route wildcard de zona.** Es lo que exige worker-as-origin con custom hostnames, pero tiene un gotcha operacional permanente: **cada subdominio first-party que no deba ejecutar `zenguy-api-production`** (p. ej. `admin.zenguy.com` o un futuro proyecto de Pages en `x.zenguy.com`) **necesita una route específica de exclusión** (Worker=None). Un Custom Domain por sí solo no tiene precedencia sobre `*/*`: sin la exclusión, el wildcard se lo traga antes de llegar a su worker de origen. Esto ya nos mordió el 30-08: al crear `*/*` capturó `admin.zenguy.com` (arreglado con la exclusión en el dash) y `api-staging.zenguy.com`, que estuvo unas horas sirviéndose desde el worker de **producción** (arreglado con la route explícita en el env staging y `wrangler triggers deploy --env staging`, que aplica routes/crons de la config sin subir código). Hay dos mecanismos válidos y **nunca los dos para el mismo patrón**: exclusión Worker=None en el dash, o route explícita en la config del worker destino — si ya existe la exclusión, el deploy de ese worker falla con "Can't deploy routes that are assigned to another worker".
...
- **Añadir un subdominio first-party nuevo:** si no debe ejecutar `zenguy-api-production`, crear ANTES su protección: exclusión Worker=None en el dash (Pages, hosts sin worker) o route explícita `host/*` en la config del worker destino (workers propios); configurar solo un Custom Domain no basta (ver gotcha del wildcard). Mantener siempre la exclusión `admin.zenguy.com/*` (panel) y la route `api-staging.zenguy.com/*` del env staging.
- **Comprobar quién sirve un host sin fiarse del health:** el CORS lo delata — el worker de producción solo devuelve `access-control-allow-origin` para `Origin: https://app.zenguy.com` y el de staging solo para `https://staging-app.zenguy.com`. `admin.zenguy.com` no se puede comprobar desde fuera (Cloudflare Access responde 302 antes del worker).
