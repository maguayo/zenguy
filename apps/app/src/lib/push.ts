...
const PUSH_LINK =
  /^zenguy:\/\/(w\/[A-Za-z0-9_-]{1,64}\/(?:overview|incidents|alerts|tests|uptime|runs|notifications)(?:\/[A-Za-z0-9_-]{1,64})?)\/?$/u;
 ...
 * path. Anything that is not a known zenguy:// workspace route is ignored so a
 * crafted payload can never steer the app elsewhere.
 */
export function pushLinkToPath(url: unknown): string | null {
  if (typeof url !== "string") return null;
  const match = PUSH_LINK.exec(url.trim());
