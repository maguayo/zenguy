...
const visit = (screen: string, resourceId?: string): ClientEvent => ({
  type: resourceId ? "browser_test.viewed" : "app.screen_viewed",
  workspaceId: "ws_1",
...
describe("activity queue", () => {
  ...
  it("dedupes app.opened without a screen property", () => {
    const { queue, sent, advance } = harness();
    const opened: ClientEvent = { type: "app.opened", properties: { appVersion: "1.2.0", platform: "ios" } };
    queue.push(opened);
    queue.push(opened);
    advance(1_000);
