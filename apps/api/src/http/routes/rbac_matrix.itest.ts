...
const ROUTES: RouteCase[] = [
  ...
  route(
    ...
    json("POST", { token: VERIFY_TOKEN, password: PASSWORD }),
  ),
  route(
  ...
  {
    label: "POST /api/invitations/preview",
    access: "P",
    success: 200,
    request: async (fixture) => ({
      path: "/api/invitations/preview",
      init: json("POST", { token: fixture.invitationToken }),
    }),
  },
  {
    label: "POST /api/invitations/accept",
    access: "A",
    success: 200,
    request: async (fixture) => ({
