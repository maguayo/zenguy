...
export class PublishStatusPage {
  constructor(
  ...
  async execute(input: {
    ...
    // Publishing twice keeps the original publication timestamp.
    const publishedAt = input.publish ? (page.publishedAt ?? now) : null;
    await this.pages.setPublished(page.id, publishedAt, now);
    await this.audit.execute({
      workspaceId: input.workspaceId,
      actorUserId: input.actor.id,
      action: input.publish
        ? AUDIT_ACTIONS.statusPagePublished
        : AUDIT_ACTIONS.statusPageUnpublished,
      resourceType: "status_page",
      resourceId: page.id,
      metadata: { slug: page.slug },
      ip: input.ip,
    });
    return { ...page, publishedAt, updatedAt: now };
  }
}
