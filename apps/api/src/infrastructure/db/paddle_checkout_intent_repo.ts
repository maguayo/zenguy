...
function toIntent(row: IntentRow): PaddleCheckoutIntent {
  return {
    id: row.id,
...
export class D1PaddleCheckoutIntentRepo implements PaddleCheckoutIntentRepo {
  ...
  async findById(id: string): Promise<PaddleCheckoutIntent | null> {
    const row = await one<IntentRow>(
      this.database
        .prepare("SELECT * FROM paddle_checkout_intents WHERE id = ?")
        .bind(id),
    );
